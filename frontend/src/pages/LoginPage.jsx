    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'linear-gradient(135deg, #F5F5F5 0%, #E8F5E9 100%)' }}>
      ...
      >
        ...
        <div className="card" style={{ padding: '2.5rem' }}>
          ...
          <p style={{ color: '#6B7280', textAlign: 'center', fontSize: '0.95rem', marginBottom: '2rem' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div>
              <label style={{ display: 'block', fontWeight: 600, fontSize: '0.875rem', color: '#374151', marginBottom: '0.5rem' }}>Password</label>
              <div style={{ position: 'relative' }}>
                <Lock size={18} style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', color: '#9CA3AF' }} />
                <input
                  type={showPass ? 'text' : 'password'} className="input-field" placeholder="••••••••"
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', margin: '1.5rem 0' }}>
            ...
            <span style={{ color: '#9CA3AF', fontSize: '0.85rem', fontWeight: 500 }}>or continue with</span>
            <div style={{ flex: 1, height: '1px', background: '#E5E7EB' }} />
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <button onClick={() => handleOAuth('google')}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', width: '100%', padding: '0.85rem', border: '2px solid #E5E7EB', borderRadius: '12px', background: 'white', cursor: 'pointer', fontWeight: 600, fontSize: '0.95rem', color: '#374151', transition: 'all 0.2s' }}
          ...
          <p style={{ textAlign: 'center', marginTop: '1.5rem', color: '#6B7280', fontSize: '0.9rem' }}>
            ...
            <Link to="/register" style={{ color: '#1B5E20', fontWeight: 700, textDecoration: 'none' }}>
              Join ReWear Free
            </Link>
          </p>
        </div>
        ...
        <div style={{ marginTop: '1rem', padding: '0.75rem 1rem', background: 'rgba(27,94,32,0.08)', borderRadius: '12px', textAlign: 'center', fontSize: '0.8rem', color: '#1B5E20' }}>
          Demo: <strong>priya@rewear.com</strong> / <strong>User@123</strong>
        </div>
      </motion.div>
    </div>
  );
